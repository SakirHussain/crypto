...
def sha512(message):
    ...
    for chunk_start in range(0, len(message_array), 128):
        ...
        for i in range(80):
            ...
            print(f"Chunk {chunk_start // 128}, iteration {i}: {hex(a)}, {hex(b)}, {hex(c)}, {hex(d)}, {hex(e)}, {hex(f)}, {hex(g)}, {hex(h)}")

        sha512_hash = [
            (x + y) & 0xFFFFFFFFFFFFFFFF
            for x, y in zip(sha512_hash, (a, b, c, d, e, f, g, h))
        ]
        # print the chunk hash
        print(f"Chunk {chunk_start // 128} hash: {sha512_hash}")
    # convert to hex and then encode to utf character string
...
if __name__ == "__main__":
    print("This is less than 896 bits")
    print(sha512("This is less than 896 bits"))
    print("This is exactly 896 bits")
    print(sha512("This line here is exactly 896 bits long. In other words it is 112 bytes long. By Guhan 21BCE1125 from crypto lab"))
    print("This is more than 896 bits")
    print(sha512("This line here is greater 896 bits long. In other words it has more than 112 characters inside it as each character is UTF-8 making it more than 896 bits long. By Guhan 21BCE1125 from crypto lab"))
