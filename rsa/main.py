...
def generate_key_pair(p, q):
    ...
    d = multiplicative_inverse(e, phi) # e * d ~ 1 mod phi(n)
    return ((e, n), (d, n))

def encrypt(pk, plaintext):
    key, n = pk
    cipher = [pow(ord(char), key, n) for char in plaintext] # message ^ e mod n ,,, n = p*q
    return cipher

def decrypt(pk, ciphertext):
    key, n = pk
    # Decrypt each chunk
    plain = [chr(pow(int(chunk), key, n)) for chunk in ciphertext] # ciphertext ^ d mod n ,,, n = p*q
    return ''.join(plain)
